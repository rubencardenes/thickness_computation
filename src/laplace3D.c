...
int laplace3D(unsigned char* input,int height, int width, int depth, float*** output, int iterations, float lambda) {
  ...
  for (l=0;l<iterations;l++) {
	...
	for (i=0;i<height;i++) {
	  if (input[sum] == 2
	      && i != 0 && i != height-1 && j!=0 && j != width-1 && k != 0 && k != depth-1) {
...
int floodfill3D(unsigned char *domain, int startindex, unsigned short oldlabel, unsigned char newlabel,int height, int width,int depth) {
  ...
  int max_num_elem_mylist = height*width*depth;
  ...
  while (mylist.num_elem != 0) {
    ...
    for (z=-1;z<2;z=z+2) {
      x=0;y=0;     
      newmapindex = mapindex + height*width*z;
      xnew = maptox3d(newmapindex,height,width);
      ynew = maptoy3d(newmapindex,height,width);
      znew = maptoz3d(newmapindex,height,width);
      
      if (xnew >= 0 && xnew < width && ynew >= 0 && ynew < height && znew >=0 && znew < depth) {
...
int RelabelBoundary3D(unsigned char *domain,int height,int width,int depth){
  ...
  while (mylist.num_elem != 0) { 
	...
	for (y=-1;y<2;y++) {
	  ...
	  newmapindex = mapindex + z*height*width + y*width + x;
	  xnew = maptox3d(newmapindex,height,width);
	  ynew = maptoy3d(newmapindex,height,width);
	  znew = maptoz3d(newmapindex,height,width);
	  if (xnew >= 0 && xnew < width && ynew >= 0 && ynew < height && znew >= 0 && znew < depth &&
  ...
  floodfill3D(domain,0,domain[0],3,height,width,depth);
  free(mylist.elem);
  return 0;
}
